...
type ProcessState struct {
	...
	// Healthcheck Endpoint. If we get http.StatusOK from this endpoint,
	// we assume the process is ready to operate. E.g. "/healthz". If
	// this is set,we ignore StartMessage.
	HealthCheckEndpoint string

	// HealthCheckPollInterval is the interval which will be used for
	// polling the HealthCheckEndpoint.
	//
	// If left empty it will default to 100 Milliseconds.
	HealthCheckPollInterval time.Duration

	// StartMessage is the message to wait for on stderr. If we
	// receive this message, we assume the process is ready to
	// operate. Ignored if HealthCheckEndpoint is specified.
	//
	// The usage of StartMessage is discouraged, favour
	// HealthCheckEndpoint instead!
	//
	...
	// ready holds whether the process is currently in ready state
	// (hit the ready condition) or not.
	// It will be set to true on a successful `Start()` and set to
	// false on a successful `Stop()`
	ready bool
}
...
// DoDefaulting sets the default configuration according to the
// data informed and return an DefaultedProcessInput
...
func DoDefaulting(
	...
	if path == "" {
		if name == "" {
			return DefaultedProcessInput{},
				fmt.Errorf("Must have at least name or path")
		}
...
// Start starts the process, waits for it to come up,
...
func (ps *ProcessState) Start(stdout, stderr io.Writer) (err error) {
	...
	if err != nil {
		return errors.Wrapf(
			err,
			"Failed to start %s", command.Args,
		)
	}
...
func (ps *ProcessState) Stop() error {
	...
	// gexec's Session methods (Signal, Kill, ...) do not check
	// if the Process is nil, so we are doing this here for now.
